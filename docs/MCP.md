...
| `graphql_execute`  | Runs a GraphQL **query** against the anonymous-role schema. Mutations and subscriptions are rejected. Returns `{ data, errors }`. Use it for anything `query_data` cannot express — nested relationship selections, fragments, variables.                                                                                                                                                                                                                                                                                                              |
...
| `AI_MCP_REQUIRE_ADMIN_SECRET` | `boolean` | `false`                                | When true, every POST `/mcp` request must carry the admin-secret header holding `ADMIN_SECRET` or `AI_MCP_SECRET` (timing-safe compare). 401 otherwise.                                                                                                                                                                                                                                |
| `AI_MCP_SECRET`               | `string`  | `""`                                   | Credential scoped to the MCP gate alone, accepted in the admin-secret header when the gate is on. Comma-separated list to rotate. Opens nothing else: anywhere but `/mcp` it resolves to the anonymous role.                                                                                                                                                                           |
...
The admin-secret header name comes from `ADMIN_SECRET_HEADER` (default `x-admin-secret`); the value is `AI_MCP_SECRET` or `ADMIN_SECRET`. The header must be present and match one of them for the gate to pass. Prefer `AI_MCP_SECRET` for MCP clients: it opens this gate and nothing else, whereas the admin secret opens everything — and the server logs a warning each time the admin secret is used here.
...
- **Constant-time secret check.** When `AI_MCP_REQUIRE_ADMIN_SECRET=true`, the header compare uses `crypto.timingSafeEqual` against `AI_MCP_SECRET` and `ADMIN_SECRET`. A missing or empty header never matches, and an unset `AI_MCP_SECRET` matches nothing.
- **Audit.** Each request that passes the gate writes an `admin_secret.used` record with the caller's address and which credential it was (`scope` is `mcp` for `AI_MCP_SECRET`, `all` for the admin secret) — see [Audit log](../README.md#audit-log).
...
The `headers` block is only needed when `AI_MCP_REQUIRE_ADMIN_SECRET=true`, and the value there should be `AI_MCP_SECRET` rather than the admin secret. Use the variable name you configured via `ADMIN_SECRET_HEADER` if you changed the default.
