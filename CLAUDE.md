...
Graphoria is a Bun + TypeScript monorepo (workspace, not a single package). It auto-generates GraphQL and REST APIs from database schemas and adds auth, queues, cron, remote-schema stitching, and remote-REST proxying on top. Three workspace packages: `@graphoria/server` (runtime + config-authoring surface at `@graphoria/server/config`), `@graphoria/react` (frontend SDK), plus one private playgrounds package (three apps) that builds the static HTML the server serves at `/graphiql`, `/scalar` and `/_console`.
