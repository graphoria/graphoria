...
Graphoria is a Bun + TypeScript monorepo (workspace, not a single package). It auto-generates GraphQL and REST APIs from database schemas and adds auth, queues, cron, remote-schema stitching, and remote-REST proxying on top. Four workspace packages: `@graphoria/server` (runtime + config-authoring surface at `@graphoria/server/config`), `@graphoria/react` (frontend SDK), `graphoria` (the CLI under its unscoped name), plus one private playgrounds package (three apps) that builds the static HTML the server serves at `/graphiql`, `/scalar` and `/_console`.
...
| `graphoria`              | `packages/graphoria/`   | `@graphoria/server`                          | Unscoped wrapper so `bunx graphoria init` works: bin `graphoria` imports the server's `cli.ts`. Pins the server at its own exact version (`version:set` moves the pin; `cli.test.ts` fails on drift). Publish after the server. |
| `@graphoria/playgrounds` | `packages/playgrounds/` | `react`, `graphiql`, `@scalar/api-reference` | Private. Three apps (`graphiql/`, `scalar/`, `console/`) bundled by `build.ts` with Bun; single-file builds → `packages/server/playgrounds/{graphiql,scalar,console}/index.html` via server's `build:playgrounds` script.       |
...
| Tracing (OTLP tracer, HTTP wrapper)                             | `observability/{tracing,httpTracing}.ts`; `configureTracing` runs at boot in `index.ts`; the active span rides an `AsyncLocalStorage`             |
...
| Verb     | Path            | Notes                                                                                                                                                                                                                                 |
| -------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET/POST | `/graphql`      | GraphQL over HTTP, POST only (queries and mutations). GET is the WebSocket upgrade (graphql-ws protocol); a plain GET answers `404`, so clients must not send queries as GET (urql: `preferGetMethod: false`).                        |
